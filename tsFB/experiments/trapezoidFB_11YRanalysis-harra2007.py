...
if __name__ == '__main__':
    ...
    final_res = (dt.datetime(year=2004,month=11,day=6,hour=0),dt.datetime(year=2004,month=11,day=13,hour=0))
    ...
    fb_vis.stack_subdecomp(data=mag_df[scalar_params+mag_components+['BY_GSE','BZ_GSE']+['BZ_GSE','BX_GSE']+['proton_density','T']],
                           main_filter=fltbnk3.fb_matrix[-1],
                           sub_filter=fltbnk4.fb_matrix[-2],
                           fftfreq=fltbnk3.freq_spectrum['sample_rate_frac'],
                           cadence=dt.timedelta(minutes=1),
                           ...
                           main_alpha=0.3,
                           sub_alpha=0.9,
                           y_labels=[y_labs['scalar_params']]+[y_labs['mag_components']]+[('By (nT)','Bz (nT)')]+[('Bz (nT)','Bx (nT)')]+[('Density (#/cm$^3$)','Temperature (K)')],
                           colors=[('blue','red'),('turquoise','orange'),('orange','orchid'),('orchid','turquoise'),('purple','green')],
                           ylims=[[(-20,20),(-120,120)],
                                  [(-25,25),(-30,30)],
                                  [(-30,30),(-50,50)],
                                  [(-50,50),(-25,25)],
                                  [(-10,25),(-450000,1250000)]],
                           date_formatter="%m-%d %H:%M",
                           rotate_xticks=7)
    ...
    fb_vis.stack_subdecomp(data=mag_df[scalar_params+['BZ_GSE','BY_GSE']+['proton_density','T']],
                           main_filter=fltbnk3.fb_matrix[-1],
                           sub_filter=fltbnk4.fb_matrix[-2],
                           fftfreq=fltbnk3.freq_spectrum['sample_rate_frac'],
                           cadence=dt.timedelta(minutes=1),
                           ...
                           main_alpha=0.3,
                           sub_alpha=0.9,
                           y_labels=[y_labs['scalar_params']]+[('Bz (nT)','By (nT)')]+[('Density (#/cm$^3$)','Temperature (K)')],
                           colors=[('blue','red'),('orchid','orange'),('purple','green')],
                           ylims=[[(-20,20),(-120,120)],
                                  [(-50,50),(-30,30)],
                                  [(-10,25),(-450000,1250000)]],
                           date_formatter="%m-%d %H:%M",
